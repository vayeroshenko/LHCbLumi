...
void groupMT()
{
	...
	input->SetBranchAddress("grannyID", grannyID);
	...
	std::vector<Int_t> vgrannyIDNew(0);
	...
	Int_t *grannyIDNew = vgrannyIDNew.data();
	...
	outTree->Branch("grannyID", grannyIDNew, "grannyID[nPart]/I");
	...
	for (Int_t i = 0; i < nFinalEvents; ++i){			// Loop for final events
		...
		vgrannyIDNew = std::vector<Int_t>(0);;
		...
		for (Int_t j = 0; j < nEvToGroup[i]; ++j){	// Loop for init events to group
			...
			for (Int_t k = 0; k < nPart; ++k) {	// Loop for particles inside event
				...
				vgrannyIDNew.push_back(grannyID[k]);
		...
		outTree->SetBranchAddress("grannyID", grannyIDNew);

...
G4VPhysicalVolume* L_DetectorConstruction::DefineVolumes(){
    ...
    //////////// CONE //////////////////////////////
        ...
        G4LogicalVolume *coneMirrorLogical = new G4LogicalVolume(coneMirrorSolid, AluminumMirr, "coneMirror");
...
//        *Ra = *Ra * RTilt;
    ...
    tabletLogical = new G4LogicalVolume(tablet,
                                        SiO2,
                                        "tablet");

    Ta = new G4ThreeVector(0.,0.,-(LConst::window_thickness + 5*mm) / 2);
    Ra = new G4RotationMatrix();

    Tr = G4Transform3D(*Ra,*Ta);

//    assembly->AddPlacedVolume(tabletLogical,Tr);


    Ta = new G4ThreeVector(0.,0.,(coneFullHeight + 0.3*mm + pmt_detector.thickness) / 2);
    Ra = new G4RotationMatrix();

    Ra->rotateX(180*deg);
    Tr = G4Transform3D(*Ra,*Ta);

        assembly->AddPlacedVolume(coneLogical,Tr);


    Ta = new G4ThreeVector(0.,0.,(coneFullHeight + pmt_detector.thickness + 2.3*mm) / 2);
    Ra = new G4RotationMatrix();

    Ra->rotateX(180*deg);
    Tr = G4Transform3D(*Ra,*Ta);

        assembly->AddPlacedVolume(coneMirrorLogical,Tr);
